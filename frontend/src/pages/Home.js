...
const AnimatedCard = ({ image, title, desc, price, delay, productId }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -15, scale: 1.02, boxShadow: '0 30px 60px rgba(233, 30, 140, 0.3)' }}
      transition={{ duration: 0.7, delay }}
      viewport={{ once: true }}
      style={{ background: 'white', borderRadius: '24px', overflow: 'hidden', boxShadow: '0 15px 40px rgba(0,0,0,0.1)', cursor: 'pointer' }}
    >
      <motion.div whileHover={{ scale: 1.1 }} transition={{ duration: 0.5 }} style={{ overflow: 'hidden', height: '280px', background: '#fff5f7' }}>
        <img
...
const Particle = ({ delay }) => (
  <motion.div
    ...
    style={{ position: 'absolute', width: '8px', height: '8px', background: '#e91e8c', borderRadius: '50%', left: `${Math.random() * 100}%`, bottom: 0 }}
  />
);
    ...
    <div style={{ overflow: 'hidden' }}>
      <section style={{ height: '100vh', position: 'relative', background: 'linear-gradient(135deg, #fff5f7 0%, #fce4ec 50%, #fff5f7 100%)', display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 10, padding: '0 8%', maxWidth: '700px' }}>
          <AnimatePresence mode="wait">
            <motion.h1 key={currentSlide} initial={{ opacity: 0, y: 50, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -50, scale: 0.9 }} transition={{ duration: 0.8 }}
              style={{ fontSize: 'clamp(3rem, 8vw, 5rem)', fontFamily: "'Playfair Display', serif", color: '#880e4f', marginBottom: '1.5rem', lineHeight: 1.1 }}>{heroTexts[currentSlide]}</motion.h1>
          </AnimatePresence>
        ...
        <motion.div animate={{ y: [0, 15, 0] }} transition={{ duration: 2, repeat: Infinity }} style={{ position: 'absolute', bottom: '3rem', left: '50%', transform: 'translateX(-50%)', zIndex: 10 }}>
          <div style={{ width: '35px', height: '55px', border: '2px solid #e91e8c', borderRadius: '18px', display: 'flex', justifyContent: 'center', paddingTop: '12px' }}>
            <motion.div animate={{ y: [0, 18, 0], opacity: [1, 0, 1] }} transition={{ duration: 1.5, repeat: Infinity }} style={{ width: '6px', height: '6px', background: '#e91e8c', borderRadius: '50%' }} />
          </div>
        </motion.div>
      </section>

      <section style={{ padding: '1.5rem 0', background: '#880e4f', overflow: 'hidden' }}>
        <motion.div animate={{ x: ['0%', '-50%'] }} transition={{ duration: 20, repeat: Infinity, ease: 'linear' }} style={{ display: 'flex', gap: '3rem', whiteSpace: 'nowrap' }}>
          {[...Array(2)].map((_, i) => (
            <div key={i} style={{ display: 'flex', gap: '3rem', alignItems: 'center', color: '#fce4ec', fontSize: '1.2rem', fontWeight: '600' }}>
              <span>Custom Cookies</span><span style={{ color: '#f8bbd0' }}>★</span><span>Gourmet Cupcakes</span><span style={{ color: '#f8bbd0' }}>★</span><span>Wedding Cakes</span><span style={{ color: '#f8bbd0' }}>★</span><span>Fresh Pastries</span><span style={{ color: '#f8bbd0' }}>★</span>
              <span>Custom Cookies</span><span style={{ color: '#f8bbd0' }}>★</span><span>Gourmet Cupcakes</span><span style={{ color: '#f8bbd0' }}>★</span><span>Wedding Cakes</span><span style={{ color: '#f8bbd0' }}>★</span><span>Fresh Pastries</span><span style={{ color: '#f8bbd0' }}>★</span>
            </div>
          ))}
        </motion.div>
      ...
      <section style={{ padding: '7rem 5%', background: 'white' }}>
        ...
        {allProducts.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} style={{ textAlign: 'center', marginTop: '3rem' }}>
            <Link to="/menu" style={{ display: 'inline-block', padding: '1rem 3rem', background: '#e91e8c', color: 'white', textDecoration: 'none', borderRadius: '50px', fontSize: '1.1rem', fontWeight: '600', boxShadow: '0 8px 25px rgba(233, 30, 140, 0.4)' }}>View Our Menu →</Link>
          </motion.div>
        )}
      </section>

      <section style={{ padding: '6rem 5%', background: 'linear-gradient(135deg, #fff5f7 0%, #fce4ec 100%)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
          {[{ icon: '🎂', title: 'Custom Designs', desc: 'Personalized for your special day' }, { icon: '🌿', title: 'Fresh Ingredients', desc: 'Only the finest quality' }, { icon: '🚚', title: 'Fast Delivery', desc: 'On-time, every time' }, { icon: '💝', title: 'Made with Love', desc: 'Every bite is special' }].map((f, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} whileHover={{ y: -10, scale: 1.05 }} transition={{ duration: 0.6, delay: i * 0.15 }} viewport={{ once: true }}
              ...
              <h3 style={{ fontSize: '1.3rem', fontFamily: "'Playfair Display', serif", color: '#880e4f', marginBottom: '0.5rem' }}>{f.title}</h3>
      ...
      <section style={{ padding: '7rem 5%', background: '#880e4f', textAlign: 'center' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
          {[{ name: 'Sarah M.', text: 'Best cupcakes in Denver! The buttercream is absolutely divine.', stars: 5 }, { name: 'John D.', text: "Ordered a custom cake for my daughter's birthday. Perfect!", stars: 5 }, { name: 'Emily R.', text: 'The sugar cookies were a hit at our wedding. Thank you!', stars: 5 }].map((r, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} whileHover={{ y: -10 }} transition={{ duration: 0.6, delay: i * 0.2 }} viewport={{ once: true }}
              ...
              <p style={{ color: '#f8bbd0', marginBottom: '1.5rem', lineHeight: 1.7, fontStyle: 'italic' }}>"{r.text}"</p>
              <p style={{ color: '#fce4ec', fontWeight: '600' }}>{r.name}</p>
            </motion.div>
          ))}
        </div>
      </section>

      <section style={{ padding: '7rem 5%', background: 'linear-gradient(135deg, #e91e8c 0%, #c2185b 100%)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
