    ...
    <div style={{ paddingTop: '100px', minHeight: '100vh', background: '#fdf8f3' }}>
      <section style={{ padding: '5rem 5%', textAlign: 'center', background: 'linear-gradient(135deg, #fdf8f3 0%, #f5deb3 100%)' }}>
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
          style={{ fontSize: 'clamp(2.5rem, 6vw, 4rem)', fontFamily: "'Playfair Display', serif", color: '#2c1810', marginBottom: '1rem' }}>Contact Us</motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
          style={{ color: '#5a3d2b', fontSize: '1.2rem' }}>We'd love to hear from you!</motion.p>
      </section>
      ...
      <section style={{ padding: '5rem 5%', display: 'flex', gap: '4rem', flexWrap: 'wrap', maxWidth: '1200px', margin: '0 auto' }}>
        <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }} style={{ flex: '1', minWidth: '300px' }}>
          <h2 style={{ fontSize: '2rem', fontFamily: "'Playfair Display', serif", color: '#2c1810', marginBottom: '2rem' }}>Get In Touch</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            {[{ icon: '📍', title: 'Address', content: 'South Denver, Colorado' }, { icon: '📞', title: 'Phone', content: '(555) 123-4567' }, { icon: '✉️', title: 'Email', content: 'hello@sweetcrumb.com' }, { icon: '⏰', title: 'Hours', content: 'Tue-Sat: 7am-6pm\nSun: 8am-3pm' }].map((info, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }}
                ...
                <div><h4 style={{ color: '#2c1810', marginBottom: '0.3rem' }}>{info.title}</h4><p style={{ color: '#666', whiteSpace: 'pre-line' }}>{info.content}</p></div>
              </motion.div>
            ))}
          </div>
        ...
        <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.2 }} viewport={{ once: true }} style={{ flex: '1', minWidth: '350px' }}>
          <form onSubmit={handleSubmit} style={{ background: 'white', padding: '2.5rem', borderRadius: '24px', boxShadow: '0 15px 40px rgba(0,0,0,0.1)' }}>
            <h3 style={{ fontSize: '1.5rem', fontFamily: "'Playfair Display', serif", color: '#2c1810', marginBottom: '1.5rem' }}>Send us a message</h3>
            ...
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit"
              style={{ width: '100%', padding: '1rem', background: submitted ? '#4CAF50' : '#d4a574', color: 'white', border: 'none', borderRadius: '12px', fontSize: '1.1rem', fontWeight: '600', cursor: 'pointer' }}>
